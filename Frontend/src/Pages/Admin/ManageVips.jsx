...
import { Plus, MoveUp, MoveDown, Trash2, Edit, Save, X, Upload, ChevronDown, ChevronUp, Play, Image as ImageIcon } from 'lucide-react';
...
const ManageVips = () => {
  ...
  const [imageForm, setImageForm] = useState({ title: '', src: '', type: 'image' });
  ...
  const [topVideoForm, setTopVideoForm] = useState({ title: '', src: '', thumbnail: '', type: 'video' });

  const topVideosSection = vipsData.find((section) => section.title === TOP_VIDEOS_SECTION_TITLE);
  const visibleSections = vipsData.filter((section) => section.title !== TOP_VIDEOS_SECTION_TITLE);
  ...
  const handleImageSave = (sectionId) => {
    if (editingImage) {
      updateVipImage(sectionId, editingImage._id, imageForm);
    } else {
      addImageToVips(sectionId, imageForm);
    }
    resetImageForm();
  };
  ...
  const ensureTopVideosSection = async () => {
    if (topVideosSection) return topVideosSection;
    return addVipSection(TOP_VIDEOS_SECTION_TITLE);
  };

  const handleTopVideoSave = async () => {
    if (!topVideoForm.src) return;

    const section = await ensureTopVideosSection();
    if (!section?._id) return;

    if (editingTopVideo) {
      await updateVipImage(section._id, editingTopVideo._id, { ...topVideoForm, type: 'video' });
    } else {
      await addImageToVips(section._id, {
        ...topVideoForm,
        title: topVideoForm.title.trim(),
        type: 'video'
      });
    }
    resetTopVideoForm();
    ...
    if (file) {
      ...
      reader.onloadend = () => {
        setter(prev => ({
          ...prev,
          [field]: reader.result,
          ...(field === 'src' ? { type: file.type.startsWith('video') ? 'video' : 'image' } : {})
        }));
      };
  ...
  return (
    <div className="space-y-8">
      <div className="flex justify-between items-center">
        <div>
          <p className="text-stone-600">Manage VIP gallery sections, captions, and photo order.</p>
          <p className="text-sm text-stone-500 mt-1">
            Top videos can be uploaded directly here. No separate section creation is needed.
          </p>
        </div>
        <button
      ...
      <div className="bg-white border border-stone-200 rounded-2xl overflow-hidden shadow-sm">
        <div className="bg-stone-50 p-4 border-b border-stone-200 flex items-center justify-between gap-4">
          <div>
            <h3 className="text-xl font-serif font-bold text-stone-800">Top Videos</h3>
            <p className="text-sm text-stone-500">These videos appear in the 4-card strip near the top of the VIP page.</p>
          </div>
          <button
            onClick={() => {
          ...
          {(isAddingTopVideo || editingTopVideo) && (
            <div className="mb-8 p-4 bg-stone-50 rounded-xl border border-stone-200 space-y-4 animate-in fade-in slide-in-from-top-2 duration-300">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                  <label className="block text-sm font-medium text-stone-700 mb-1">Video Title (Optional)</label>
                  <input
                    type="text"
                    value={topVideoForm.title}
                    onChange={(e) => setTopVideoForm({ ...topVideoForm, title: e.target.value })}
                    className="w-full px-4 py-2 border border-stone-300 rounded-lg outline-none"
                    placeholder="Optional caption"
                  />
                </div>
                <div>
                  <label className="block text-sm font-medium text-stone-700 mb-1">Video Upload or URL</label>
                  <div className="flex gap-2">
                    <input
                      type="text"
                      value={topVideoForm.src.startsWith('data:') ? 'Local file selected' : topVideoForm.src}
                      onChange={(e) => setTopVideoForm({ ...topVideoForm, src: e.target.value, type: 'video' })}
                    ...
                    <label className="bg-white border border-stone-300 px-4 py-2 rounded-lg cursor-pointer hover:bg-stone-50 transition-colors flex items-center gap-2">
                      ...
                      <input
                        ...
                        className="hidden"
                        accept="video/*"
                        onChange={(e) => handleFileChange(e, setTopVideoForm, topVideoForm, 'src')}
                      />
                    </label>
                  </div>
                </div>
                <div>
                  <label className="block text-sm font-medium text-stone-700 mb-1">Thumbnail Image (Optional)</label>
                  <div className="flex gap-2">
                    <input
                      type="text"
                      value={topVideoForm.thumbnail?.startsWith('data:') ? 'Image selected' : (topVideoForm.thumbnail || '')}
                      onChange={(e) => setTopVideoForm({ ...topVideoForm, thumbnail: e.target.value })}
                      className="flex-1 px-4 py-2 border border-stone-300 rounded-lg outline-none"
                    ...
                    <label className="bg-white border border-stone-300 px-4 py-2 rounded-lg cursor-pointer hover:bg-stone-50 transition-colors flex items-center gap-2">
                      ...
                      <input
                        ...
                        className="hidden"
                        accept="image/*"
                        onChange={(e) => handleFileChange(e, setTopVideoForm, topVideoForm, 'thumbnail')}
                      />
                    </label>
                  </div>
                  {topVideoForm.thumbnail && (
                    <img src={topVideoForm.thumbnail} className="mt-2 h-16 rounded object-cover" alt="preview" />
                  )}
                </div>
              </div>
          ...
          {topVideosSection?.images?.length > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              {topVideosSection.images.map((video, videoIndex) => (
                <div key={video._id} className="relative group bg-white border border-stone-100 rounded-xl p-3 shadow-sm hover:shadow-md transition-all">
                    ...
                    <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2">
                      <button
                        onClick={() => {
                          ...
                          setIsAddingTopVideo(false);
                          setTopVideoForm({ title: video.title || '', src: video.src, thumbnail: video.thumbnail || '', type: 'video' });
                        }}
                  ...
                  <div className="flex items-center gap-2">
                    <div className="flex-1">
                      <h5 className="font-bold text-sm text-stone-900 truncate">{video.title || `Video ${videoIndex + 1}`}</h5>
                    </div>
                    <div className="flex items-center gap-1 bg-stone-50 px-2 py-1 rounded-lg border border-stone-200">
                      <input
                        ...
                        onBlur={(e) => {
                          ...
                          if (targetIdx !== videoIndex && targetIdx >= 0 && targetIdx < topVideosSection.images.length) {
                            reorderVipImage(topVideosSection._id, videoIndex, targetIdx);
                          }
                        }}
                      ...
                      <div className="flex flex-col border-l border-stone-200 pl-1">
                        <button
                          ...
                          onClick={() => moveVipImage(topVideosSection._id, videoIndex, -1)}
                          className="p-0.5 hover:bg-stone-200 rounded disabled:opacity-20"
                        >
                          <MoveUp size={10} />
                        </button>
                        <button
                          disabled={videoIndex === topVideosSection.images.length - 1}
                          onClick={() => moveVipImage(topVideosSection._id, videoIndex, 1)}
                          className="p-0.5 hover:bg-stone-200 rounded disabled:opacity-20"
                        >
                          <MoveDown size={10} />
      ...
      <div className="space-y-6">
        {visibleSections.map((section, sectionIndex) => {
          const actualSectionIndex = vipsData.findIndex((item) => item._id === section._id);
          ...
          <div key={section._id} className="bg-white border border-stone-200 rounded-2xl overflow-hidden shadow-sm">
            <div className="bg-stone-50 p-4 border-b border-stone-200 flex items-center gap-4">
              <div className="flex flex-col">
                ...
                <button
                  disabled={actualSectionIndex === -1 || actualSectionIndex >= vipsData.length - 1}
                  onClick={() => moveVipSection(actualSectionIndex, 1)}
                  className="p-1 hover:bg-stone-200 rounded disabled:opacity-20 transition-colors"
                >
                  <MoveDown size={16} />
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      ...
                      <div>
                        ...
                        <div className="flex gap-2">
                          <input
                            type="text"
                            value={imageForm.src.startsWith('data:') ? 'Local file selected' : imageForm.src}
                          ...
                          <label className="bg-white border border-stone-300 px-4 py-2 rounded-lg cursor-pointer hover:bg-stone-50 transition-colors flex items-center gap-2">
                            ...
                            <input
                              ...
                              className="hidden"
                              accept="image/*,video/*"
                              onChange={(e) => handleFileChange(e, setImageForm, imageForm, 'src')}
                            />
                          </label>
                        </div>
                        {imageForm.type === 'video' && <p className="text-[10px] text-red-700 mt-1 font-bold">Video detected</p>}
                      </div>
                    </div>
                    <div className="flex justify-end gap-2">
                      <button
                          ...
                          <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2">
                            <button
                              onClick={() => {
                                ...
                                setImageForm({ title: img.title, src: img.src, type: img.type || 'image' });
                              }}
                              className="p-2 bg-white rounded-full text-stone-900 hover:bg-red-50 hover:text-red-700"
                            >
                              <Edit size={16} />
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <div className="flex items-center gap-1 bg-stone-50 px-2 py-1 rounded-lg border border-stone-200">
                            <input
                              ...
                              onBlur={(e) => {
                                ...
                                if (targetIdx !== imgIndex && targetIdx >= 0 && targetIdx < section.images.length) {
                                  reorderVipImage(section._id, imgIndex, targetIdx);
                                }
                              }}
                              className="w-8 text-xs text-center bg-transparent border-none outline-none focus:ring-0 font-bold"
                            />
                            <div className="flex flex-col border-l border-stone-200 pl-1">
                              <button
                                disabled={imgIndex === 0}
                                onClick={() => moveVipImage(section._id, imgIndex, -1)}
                                className="p-0.5 hover:bg-stone-200 rounded disabled:opacity-20"
                              >
                                <MoveUp size={10} />
                              </button>
